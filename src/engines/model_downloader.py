...
class ModelDownloader:
    ...
    def __init__(self, bucket_name: str = MODEL_BUCKET_NAME):
        ...
        self.remote_prefix = "models/"

    def run(self):
        """
        GCS로부터 모델 다운로드 실행
        """
        print(f"🔍 GCS 버킷 '{self.bucket_name}'에서 모델 목록을 가져오는 중...")
        blobs = self.storage_service.list_blobs(self.bucket_name, prefix=self.remote_prefix)
        
        if not blobs:
            print(f"❌ '{self.remote_prefix}' 경로에 저장된 모델 데이터가 없습니다.")
            return

        print(f"🚀 모델 다운로드 시작 (총 {len(blobs)}개 파일)")
        
        count_downloaded = 0
        count_failed = 0

        # tqdm 진행바 설정
        progress_bar = tqdm(blobs, desc="모델 다운로드 중", unit="파일", leave=True)

        for blob in progress_bar:
            remote_path = blob.name
            # models/ -> models/ (로컬 루트 하위)
            # remote_path가 'models/players/ai_lv1.pth' 라면 로컬 'models/players/ai_lv1.pth'로 저장
            relative_path = remote_path[len(self.remote_prefix):]
            if not relative_path: # 폴더 자체인 경우 건너뜀
                continue
            
            local_path = os.path.join(self.model_dir, relative_path)
